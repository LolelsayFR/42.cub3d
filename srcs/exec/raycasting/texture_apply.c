...
/*   Updated: 2025/05/27 09:12:01 by emaillet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	texture_apply(t_img *img, int x, int y, t_ray ray)
{
	...
	if (ray.wally <= 0 || imgy <= 0)
		return ;
	pixel = ray.texture->data + ((imgy % ray.texture->height)
			* ray.texture->size_line + (imgx % ray.texture->width)
			* (ray.texture->bpp / 8));
	color = darker_rgb(*(unsigned int *)pixel, ray.dist);
